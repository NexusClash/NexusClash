Feature: Combat
  ...
  Scenario: Sally tries to attack Joe but has no ammo
    Given a character named Joe
    And a character named Sally
    Given Joe has an innate ammo-using ranged attack (5 impact @ 100%) loaded with 0 ammo
    And Joe has 40 HP
    When Sally attacks Joe with her weapon
    Then Joe should have 40 HP
  ...
  Scenario: Sally tries to attack Joe and has ammo
    Given a character named Joe
    And a character named Sally
    Given Joe has an innate ammo-using ranged attack (5 impact @ 100%) loaded with 1 ammo
    And Joe has 40 HP
    When Sally attacks Joe with her weapon
    Then Joe should have 35 HP
  ...
  Scenario: Sally tries to attack Joe twice but only has enough ammo for 1 shot
    ...
    And Joe has 40 HP
    When Sally attacks Joe with her weapon
    And Sally attacks Joe with her weapon
    Then Joe should have 35 HP