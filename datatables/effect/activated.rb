module Effect
	class Activated
		...
		def save_state_to_datatable
			{type: 'Activated', text_2: @costs[:ap], text_3: @costs[:mp], text_1: @name}
		end

		def self.save_state_from_datatable(parent, table)
			if table[:text_1] != ''
				['Activated', {ap: table[:text_2].to_i, mp: table[:text_3].to_i}, table[:text_1]]
			else
				['Activated', {ap: table[:text_2].to_i, mp: table[:text_3].to_i}]
			end
		end
