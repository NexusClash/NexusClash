module Effect
	class Defence
		...
		def save_state_to_datatable
			...
			{type: 'Defence', text_1: @name, select_1: @type, text_2: soak_string.chomp(','), text_3: resistances_string.chomp(','), text_4: @avoidance[DefenceType::GENERIC_COMBAT_AVOIDANCE], text_5: @avoidance[DefenceType::CLOSE_COMBAT_AVOIDANCE], text_6: @avoidance[DefenceType::RANGED_COMBAT_AVOIDANCE]}
		end
		...
		def self.save_state_from_datatable(parent, table)
			...
			if table[:text_1] == '' || (parent.respond_to?(:name) && table[:text_1] == parent.name)
				['Defence', nil, s_soaks, s_res, avoidance, table[:select_1].to_sym]
			else
				['Defence', table[:text_1], s_soaks, s_res, avoidance, table[:select_1].to_sym]
			end
		end
